...
def getCritDist(L, p, A1, A2, D1, D2, B, stab, te):
    # Fractions used here exactly like mdp_clean
    p = min(p, Fraction(1))
    norm = getDamages(L, A1, D1, B, stab, te)
    crit = getDamages(L * 2, A2, D2, B, stab, te)

    dist = collections.defaultdict(Fraction)
    mult_norm = (Fraction(1) - p) / len(norm)
...
class Battle(object):
    ...
    def _getSuccessorsB(self, statep):
        ...
        pairs = sorted(
            ((k, float(p)) for k, p in dist.items() if p > 0),
    ...
    def getSuccessors(self, statep):
        ...
        if st == 0:
            ...
            if st == 1:
                pairs = self._getSuccessorsB(statep)   # list of (statep, float)
            else:
    ...
    def build_graph(self, initial_statep):
        ...
        succ_pairs  = []    # for st in {1,2}: list[(id, float)]
        ...
        while q:
            ...
            if st == 0:
                ...
                nxt = self.getSuccessors(sp)   # list[(statep, float)]
                pairs = []
                for sp2, p in nxt:
                    if sp2 not in id_of:
                        id_of[sp2] = len(states); states.append(sp2); q.append(sp2)
